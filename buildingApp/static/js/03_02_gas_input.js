function showLeaseInfo(isReload)
{
	...
	if (isReload) {
		...
		if (curType == '전기') tyep_text = 'electricity';
		else if (curType == '가스') type_text = 'gas';
		else if (curType == '수도') type_text = 'water';
	}
...
var doAjaxContents_G = function(roomnum) {
	...
	$.ajax({
		...
		success : function(result) {
			EGW_G = result;
			var template = new EJS({url : '/static/ejs/03_02_gas.ejs'}).render({'data' : EGW_G, 'floor' : ''});
...
{
	var template = new EJS({url : '/static/ejs/03_02_gas.ejs'}).render({'data' : EGW_G, 'floor' : val});
	$('#contents').html(template);
	$('#contents_modal').html(template);
}
