...
def resident_show_html(request):
    csrf_token = get_token(request)
    return render(request, '02_02_resident_show.html')
        ...
        if int(param['type']) == 0:
            ...
            if re.sub('[0-9]+', '', word) == '':
                result = ResidentInfo.objects.filter(
                    Q(buildingName=word) | Q(buildingRoomNumber=word) | Q(contractorName=word))
            else:
                result = ResidentInfo.objects.filter(
                    Q(buildingName=word) | Q(contractorName=word))

            #t = loader.get_template('02_02_resident_show.html')
            #c = RequestContext(request, {'result' : json.dumps(serialize(result), ensure_ascii=False)})
            #t.render(c)
            #return HttpResponse(t.render(c), content_type='application/json; charset=utf-8')
            #return render(request, '02_02_resident_show.html',
            #        {'result' : json.dumps(serialize(result), ensure_ascii=True)}, content_type='application/json; charset=utf-8')
            ...
            for p in param:
                if param[p] != '':
                    if p == 'buildingName':
                        result = result.filter(buildingName = param[p])
                    ...
                    #elif p == '':
                    #    result = result.filter(isEmptyRoom = param[p])
                    #elif p == '':
                    #    result = result.filter(isOverdue = param[p])
...
def serialize(result):
    ...
    for res in result:
        ...
        '''
        for d in data:
            if d in ['leaseMoney', 'leaseDeposit', 'buildingRoomNumber']:
                continue
            #print(d, data[d])
            #data[d] = str(data[d].encode('utf8')) #data[d].encode('ascii', 'ignore')
            #print(data[d])
        '''
...
def show_detail_resident_info(request, uid):
    ...
    return render(request, '02_02_resident_show_detail.html', {'result' : result, 'range' : range(1, 32)})
